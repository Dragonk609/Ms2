...
void removePatient(struct Patient patient[], int max) {
    int valid_input = 1;
    int pNum, index;
    char choice, check; // check is for choice have multiple letter
    printf("Enter the patient number: ");
    scanf("%d", &pNum);
    printf("\n");
    ...
    if (findPatientIndexByPatientNum(pNum, patient, max) != -1) {
        ...
        while (valid_input != 0) {
            ...
            if ((choice == 'n' || choice == 'N') && (check == '\n')) {
                printf("Operation aborted.\n\n");
                valid_input = 0;
                clearInputBuffer();
            }
            else if ((choice == 'y' || choice == 'Y') && (check == '\n')) {
    ...
    if (index >= 0) {
        while (slotNotAvailable)
        {
            ...
            if (timeSlotAvailable(date, time, app, maxAppointments)) {
                ...
                while ((time.hour < START_TIME || time.hour > END_TIME) || (time.hour == END_TIME && time.min > 0) || (time.min % MIN_INTERVAL != 0)) {
                    printf("ERROR: Time must be between %02d:00 and %02d:00 in %02d minute intervals.\n\n", START_TIME, END_TIME, MIN_INTERVAL);
                    printf("Hour (0-23)  : ");
                    time.hour = inputIntRange(0, 23);
                    printf("Minute (0-59): ");
                    time.min = inputIntRange(0, 59);
...
void removeAppointment(struct Appointment* app, int maxAppointments, struct Patient* pt, int maxPatients) {
    ...
    if (index >= 0) {
        ...
        aptIndex = validSlotAppointment(patientNumber, date, app, maxAppointments);
